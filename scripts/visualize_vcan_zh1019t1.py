#!/usr/bin/env python3
"""
VCAN spatial isoform visualization for ZH1019T1.
5-panel figure:
  A) VCAN expressing spots
  B) Tumor (Malignant) proportion from SpaCET
  C) Macrophage proportion from SpaCET
  D) VCAN isoform 1 fraction
...
def load_spatial_and_isoforms(sample_id, gene="VCAN"):
...
def create_vcan_figure(sample_id=SAMPLE, output_path=None):
    """Create 5-panel VCAN visualization."""

    print(f"Loading data for {sample_id}...")
    coords, obs_names, iso1_frac, gene_total, iso_names = \
        load_spatial_and_isoforms(sample_id, "VCAN")
    print(f"  {len(obs_names)} spots, {len(iso_names)} VCAN isoforms")
    ...
    print(f"  VCAN-expressing spots: {n_expr}/{len(common)}")
    ...
    ax_a.set_title(f'A) VCAN Expression\n({n_expr}/{len(common)} spots)',
    ...
    ax_d.set_title('D) VCAN Isoform 1 Fraction', fontsize=12, fontweight='bold')
    ax_d.invert_yaxis()
    ...
    ax_e.set_ylabel('Spearman r (vs VCAN Isoform 1 Fraction)', fontsize=12)
    ax_e.set_title('E) Cell Type Proportion vs VCAN Isoform 1 Correlation',
                   fontsize=12, fontweight='bold')
    ax_e.set_xlim(-0.8, n_ct - 0.2)
    ax_e.set_ylim(-0.3, 0.35)

    # Add significance markers above/below bars
    ...
    fig.suptitle(f'{sample_id} — VCAN Spatial Isoform Switching in Glioma TME',
                 fontsize=15, fontweight='bold', y=1.01)

    if output_path is None:
        output_path = REPORTS_DIR / f"vcan_validation_{sample_id}.png"
