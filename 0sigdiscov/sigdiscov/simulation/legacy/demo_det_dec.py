#!/usr/bin/env python3
"""
Experiment v5b: Long-Range Niche with Distributed Silent Senders
Changes from v5:
1. Active senders clustered at center (signal source)
2. Silent senders spread uniformly across domain (negative controls)
3. Tests if I_ND correctly goes to 0 at distances > lambda
"""
...
def run_experiment_v5():
    print("="*70)
    print("EXPERIMENT v5b: Distributed Silent Senders (Negative Controls)")
    print("="*70)
    ...
    for frac in receiver_fractions:
        ...
        # FIX POSITIONS: Active senders to center, Silent senders spread uniformly
...
def plot_results_v5(results, fractions, center, max_radius):
    ...
    plt.title("Niche Detection: Distributed Silent Senders", fontsize=16, fontweight='bold')
    ...
    plt.tight_layout()
    plt.savefig(f"{OUTPUT_FOLDER}/long_range_niche_distributed_silent.png", dpi=300)
    plt.close()
