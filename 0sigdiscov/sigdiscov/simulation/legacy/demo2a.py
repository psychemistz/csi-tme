#!/usr/bin/env python3
"""
Experiment v5c: Long-Range Niche with Centered Active, Distributed Silent Senders
Changes from v5:
1. Active senders clustered at center (signal source)
2. Silent senders spread uniformly across domain (negative controls)
3. Tests if I_ND correctly detects signaling from centered source
"""
...
def plot_cell_distribution(all_positions, active_indices, silent_indices, receiver_indices, center, frac_str):
    ...
    ax.set_title(f'Cell Distribution - Centered Active, Distributed Silent ({frac_str} Receivers)', fontsize=14, fontweight='bold')
...
def run_experiment_v5():
    print("="*70)
    print("EXPERIMENT v5c: Centered Active, Distributed Silent Senders")
    print("="*70)
   ...
   # --- CALCULATE REQUIRED D ---
    ...
    for frac in receiver_fractions:
        ...
        plot_cell_distribution(all_positions, active_indices, silent_indices, 
                               receiver_indices, center, frac_str)
...
def plot_results_v5(results, fractions, center, max_radius):
    ...
    plt.title("Niche Detection: Centered Active, Distributed Silent", fontsize=16, fontweight='bold')
