...
2. Concentration-dependent receiving: B_j ~ Bernoulli(p_r(C_j))
   where p_r(C) = p_r_max * C^n / (K_p^n + C^n)  (Hill function)
3. Effective receiver density: n_r_eff = n_r * <p_r> (mean p_r)
...
Response expression (mixture):
  ...
  where B_j ~ Bernoulli(p_r(C_j)) with concentration-dependent probability
"""
...
def hill_function(C, p_r_max, K_p, n):
    """
    Hill function for concentration-dependent receiving probability.
    ...
    """
    C = np.asarray(C)
    C_n = np.power(np.maximum(C, 0), n)  # Ensure non-negative
    K_n = np.power(K_p, n)
...
def generate_response_expression(n_total, receiver_indices, concentrations,
                                  R_basal, FC, Kd, sigma_r, sigma_r_b,
                                  p_r_max=1.0, K_p=10.0, hill_n=1.0):
    """
    Generate response expression using mixture model with concentration-dependent p_r.
    ...
    where B_j ~ Bernoulli(p_r(C_j)) for receiver cells, B_j = 0 for non-receivers
    Act_j = C_j / (Kd + C_j)
    
    Parameters:
    -----------
    p_r_max : float
        Maximum receiving probability (saturation level)
    K_p : float
        Concentration at half-maximal probability
    hill_n : float
        Hill coefficient
    ...
    # Calculate p_r for each receiver based on local concentration
    p_r_values = hill_function(C_receivers, p_r_max, K_p, hill_n)
    
    # Bernoulli draws with cell-specific probabilities
    ...
    # Return responding receiver indices (where B_j = 1) and mean p_r
...
def plot_results(results, fractions, p_s, hill_params, sigma_params):
    ...
    for f, col in zip(fractions, colors):
        ...
        plt.plot(dists, vals_smooth, '-', color=col, linewidth=3, 
                 label=f'{f*100:.0f}% Recv (λ={lambda_v:.0f}μm, <p_r>={mean_p_r:.2f})')
    ...
    hill_str = f"p_r(C) = {hill_params['p_r_max']} · C^{hill_params['hill_n']} / ({hill_params['K_p']}^{hill_params['hill_n']} + C^{hill_params['hill_n']})"
    plt.title(f"Stochastic Model (p_s={p_s})\n{hill_str}\n{sigma_str}", 
...
def run_stochastic_experiment(sigma_f=0.1, sigma_f_b=0.1, sigma_r=0.1, sigma_r_b=0.1,
                               p_s=1.0, p_r_max=1.0, K_p=10.0, hill_n=1.0):
    """
    ...
    p_r_max : float
        Maximum receiving probability (Hill function saturation)
    K_p : float
        Concentration at half-maximal receiving probability
    hill_n : float
        Hill coefficient (n > 1: switch-like, n = 1: hyperbolic)
    """
    print("="*70)
    ...
    print(f"Parameters:")
    print(f"  p_s = {p_s}")
    print(f"  p_r(C) = {p_r_max} * C^{hill_n} / ({K_p}^{hill_n} + C^{hill_n})")
    ...
    for frac in receiver_fractions:
        ...
        # Initial estimate using p_r_max for lambda calculation
        # (actual mean_p_r will be computed after concentration is known)
        n_density = n_receivers / domain_area
        n_density_eff_initial = n_density * p_r_max
        
        # 4. Concentration field (using initial estimate for lambda)
        concentrations, lambda_val = solve_concentration_field_MM(
            all_positions[sender_indices], factor_expr[sender_indices], 
            all_positions, n_density_eff_initial, D, k_max, Kd
        )
        
        # 5. Response Expression (mixture model with concentration-dependent p_r)
        responsive_expr, responding_receiver_indices, p_r_values, mean_p_r = generate_response_expression(
            n_total, receiver_indices, concentrations,
            R_BASAL, FOLD_CHANGE, Kd, sigma_r, sigma_r_b,
            p_r_max=p_r_max, K_p=K_p, hill_n=hill_n
        )
        
        # Recalculate lambda with actual mean_p_r for reporting
        n_density_eff_actual = n_density * mean_p_r
        lambda_val_actual = calculate_lambda(D, n_density_eff_actual, k_max, Kd)
        ...
        results[frac] = {
            'lambda': lambda_val,
            'lambda_actual': lambda_val_actual,
            'mean_p_r': mean_p_r,
            'ind_curve': ind_curve,
            'bandwidth': BANDWIDTH,
        ...
        print(f"  λ_eff (p_r_max): {lambda_val:.0f} μm")
        print(f"  λ_eff (actual): {lambda_val_actual:.0f} μm")
        print(f"  Mean p_r: {mean_p_r:.3f}")
    ...
    plot_results(results, receiver_fractions, p_s, hill_params, sigma_params)
